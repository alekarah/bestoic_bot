...
class Database:
    ...
    def get_user_favorites(self, user_id: int, limit: int = 10, offset: int = 0) -> List[Tuple]:
        ...
        Args:
            user_id: Telegram user ID
            limit: Number of quotes per page
            offset: Offset for pagination

        Returns:
            List of quote dicts with full info
        """
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
    ...
    def add_book_file(self, library_book_id: int, format: str,
        ...
        Args:
            library_book_id: ID of the library book
            format: File format (fb2, epub, mobi, pdf)
            file_id: Telegram file_id (for caching)
            file_path: Local file path
            file_size: File size in bytes

        Returns:
            file_id of the new record
        """
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
